...
{
	ofLogNotice ( "LiveView::Initialise" ) << "Called";
...
void Explorer::LiveView::UpdateAudioPlayers ( )
{
	...
	if ( mRawView->IsTimeAnalysis ( )  )
	{
		...
		for ( int i = 0; i < mPlayingFiles.size ( ); i++ )
		{
			...
			if ( !mSoundPlayers[i].isPlaying ( ) || timePlayed < mPlayingLastPositionMS[i] || ( mPlayingTimeHeads[i] + 1 ) >= time->raw[mPlayingFiles[i]].size ( ) )
			{
				if ( !bLoopAudio )
				{
	...
	else // stats
	{
		...
		for ( int i = 0; i < mPlayingFiles.size ( ); i++ )
		{
			if ( mSoundPlayers[i].isPlaying ( ) ) { continue; }
...
void Explorer::LiveView::Draw ( )
{
	...
	if ( mRawView->IsTimeAnalysis ( ) ) // Time
	{
		...
		else
		{
			...
			// Draw playheads as spheres
			for ( int i = 0; i < mPlayheads.size ( ); i++ )
			{
				ofDisableDepthTest ( );
				ofDisableAlphaBlending ( );

				ofColor color = mPlayheads[i].color; 
				float size = 50;
				if ( mPlayheads[i].highlight ) 
				{ 
	...
	{
		// Get list of playing files from active playheads
		std::set<int> playingFileIndices;
		for ( const auto& playhead : mPlayheads )
		{
			playingFileIndices.insert ( playhead.fileIndex );
		}
		
		if ( mPointPicker->GetNearestPointFile ( ) == -1 && playingFileIndices.empty() )
		{
		...
		else
		{
			...
			// Highlight all files with active playheads
			for ( int fileIndex : playingFileIndices )
			{
				if ( fileIndex >= 0 && fileIndex < mStatsCorpus.getNumVertices() )
				{
					ofSetColor ( 255, 255, 255, 150 );
					ofDrawSphere ( mStatsCorpus.getVertex ( fileIndex ), 25 );
				}
			}

			// Highlight the nearest point in its original color
			if ( mPointPicker->GetNearestPointFile ( ) >= 0 && 
			     mPointPicker->GetNearestPointFile ( ) < mStatsCorpus.getNumVertices() )
			{
	...
	// Draw playhead panels at bottom of screen
	for ( auto& playhead : mPlayheads )
	{
		...
		// Draw playhead information
		ofSetColor ( 255 );
		
		// Line 1: Playhead ID
		ofDrawBitmapString ( "Playhead " + std::to_string ( playhead.playheadID ), 
		                     playhead.panelRect.x + 5, 
		                     playhead.panelRect.y + 15 );
		
		// Line 2: File name (truncated if too long)
		if ( playhead.fileIndex < mRawView->GetDataset ( )->fileList.size ( ) )
		{
			...
			ofDrawBitmapString ( filename, 
			                     playhead.panelRect.x + 5, 
			                     playhead.panelRect.y + 30 );
		}
		
		// Line 3: Time position
		if ( mRawView->GetDataset ( ) )
		{
			float sampleRate = mRawView->GetDataset ( )->analysisSettings.sampleRate;
			float timeInSeconds = playhead.sampleIndex / sampleRate;
			ofDrawBitmapString ( ofToString ( timeInSeconds, 2 ) + "s", 
			                     playhead.panelRect.x + 5, 
			                     playhead.panelRect.y + 45 );
		}
		
		// Line 4: Status (if crossfading/jumping)
		// This would need to be added to VisualPlayhead struct to show crossfade status
	}
...
void Explorer::LiveView::UpdatePlayheads ( )
{
	...
	{
		auto it = std::find_if ( mPlayheads.begin ( ), mPlayheads.end ( ), [i, &playheadUpdates] ( Utils::VisualPlayhead& playhead ) { return playhead.playheadID == playheadUpdates[i].playheadID; } );
	...
	for ( auto& playhead : mPlayheads )
	{
		if ( !mRawView->IsTimeAnalysis() ) 
		{
			continue; // Skip position calculation for stats mode
		}
		...
		// Bounds checking
		if ( playhead.fileIndex < mTimeCorpus.size() && timeIndex < mTimeCorpus[playhead.fileIndex].getNumVertices() )
		{
...
{
	// Use GetNearestPointFile which represents the currently selected point
	if ( mPointPicker->GetNearestPointFile ( ) == -1 ) { return; }

	size_t fileIndex = mPointPicker->GetNearestPointFile();
	size_t timeIndex = mPointPicker->GetNearestPointTime();

	size_t sampleIndex = timeIndex;
	if ( mRawView->IsTimeAnalysis() && mRawView->GetDataset() )
	{
		auto* ds = mRawView->GetDataset();
		size_t hopSize = ds->analysisSettings.windowFFTSize / ds->analysisSettings.hopFraction;
		sampleIndex = timeIndex * hopSize;
	}

	mAudioPlayback.CreatePlayhead( fileIndex, sampleIndex );
...
void Explorer::LiveView::CreatePoints ( )
{
	if ( mRawView->IsTimeAnalysis ( ) )
	{
		...
		// Update AudioPlayback with time corpus
...
void Explorer::LiveView::FillDimensionNone ( Utils::Axis axis )
{
	...
	// Update AudioPlayback with time corpus if in time analysis mode
	if ( mRawView->IsTimeAnalysis ( ) )
	{
		mAudioPlayback.SetTimeCorpus ( mTimeCorpus );
	}
...
void Explorer::LiveView::MouseEvent ( ofMouseEventArgs& args )
{
	...
	// Check for clicks on playhead panels first
	if ( args.type == 0 ) // mouse pressed
	{
		for ( auto& playhead : mPlayheads )
		{
			if ( playhead.panelRect.inside ( args.x, args.y ) )
			{
				if ( args.button == 0 ) // left click - highlight/unhighlight
				{
					playhead.highlight = !playhead.highlight;
				}
				else if ( args.button == 2 ) // right click - kill playhead
				{
					KillPlayhead ( playhead.playheadID );
				}
